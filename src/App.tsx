...
import { fieldInputType, formatDate, formatDateTime, getVisibilityTone, initials, isEditableByUser, isValidCardNumber, slugify, validateValue } from './lib/utils'
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
        ...
        <div className="sidebar-bottom"><div className="help-card"><LifeBuoy size={17} /><div><strong>Need a hand?</strong><span>Check your setup guide</span></div><ArrowRight size={15} /></div><div className="profile-chip"><div className="avatar">{initials(profile.display_name, profile.email)}</div><div className="profile-meta"><strong>{profile.display_name || profile.email.split('@')[0]}</strong><span>{profile.role === 'admin' ? 'Administrator' : 'Team member'}</span></div><button className="icon-button" onClick={onSignOut} title="Sign out"><LogOut size={17} /></button></div></div>
      </aside>
...
function UsersView({ currentUser, notify, onProfileChange }: { currentUser: Profile; notify: (type: Toast['type'], message: string) => void; onProfileChange: (profile: Profile) => void }) {
  ...
  const filtered = useMemo(() => users.filter((user) => `${user.display_name || ''} ${user.email}`.toLowerCase().includes(search.toLowerCase())), [search, users])

  async function changeStatus(user: Profile) {
    try { await manageUser(user.id, 'status', !user.is_active); setUsers((current) => current.map((item) => item.id === user.id ? { ...item, is_active: !item.is_active } : item)); notify('success', `${user.email} is now ${user.is_active ? 'inactive' : 'active'}.`) } catch (error) { notify('error', getErrorMessage(error)) }
  }
  async function changeRole(user: Profile) {
    const nextRole: Role = user.role === 'admin' ? 'user' : 'admin'
    try { await manageUser(user.id, 'role', nextRole); setUsers((current) => current.map((item) => item.id === user.id ? { ...item, role: nextRole } : item)); if (user.id === currentUser.id) onProfileChange({ ...currentUser, role: nextRole }); notify('success', `${user.email} is now a ${nextRole}.`) } catch (error) { notify('error', getErrorMessage(error)) }
  }

  return <><PageHeader eyebrow="Team access" title="Keep access intentional." description="Invite teammates, set roles, and pause access without touching the customer records they created." action={<button className="button primary" onClick={() => setShowInvite(true)}><UserPlus size={17} /> Invite teammate</button>} /><div className="team-toolbar"><div className="team-summary"><div className="summary-icon"><Users size={18} /></div><div><strong>{users.filter((user) => user.is_active).length} active members</strong><span>{users.length} total accounts in this workspace</span></div></div><div className="search-box small"><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search people…" /></div></div>{loading ? <LoadingBlock /> : <div className="panel people-panel"><div className="table-heading"><span>Person</span><span>Role</span><span>Status</span><span>Joined</span><span /></div>{filtered.map((user) => <div className="person-row" key={user.id}><div className="person-cell"><div className="avatar">{initials(user.display_name, user.email)}</div><div><strong>{user.display_name || 'Pending teammate'}</strong><span>{user.email}</span></div></div><div><span className={`role-pill ${user.role}`}><span />{user.role}</span></div><div><span className={`status-pill ${user.is_active ? 'active' : 'archived'}`}><span />{user.is_active ? 'Active' : 'Inactive'}</span></div><div className="joined-date">{formatDate(user.created_at)}</div><div className="row-menu"><button className="icon-button" title="Toggle access" disabled={user.id === currentUser.id} onClick={() => void changeStatus(user)}>{user.is_active ? <Lock size={16} /> : <KeyRound size={16} />}</button><button className="icon-button" title="Change role" disabled={user.id === currentUser.id} onClick={() => void changeRole(user)}><MoreHorizontal size={17} /></button></div></div>)}{!filtered.length && <EmptyState compact icon={<Users size={20} />} title="No teammates found" description="Try another search term." />}</div>}{showInvite && <InviteModal onClose={() => setShowInvite(false)} onInvite={async (email, name, role) => { await inviteUser(email, name, role); setShowInvite(false); notify('success', `Invitation sent to ${email}.`); await reload() }} />}</>
}

function InviteModal({ onClose, onInvite }: { onClose: () => void; onInvite: (email: string, name: string, role: Role) => Promise<void> }) {
  const [email, setEmail] = useState('')
  const [name, setName] = useState('')
  const [role, setRole] = useState<Role>('user')
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState('')
  return <Modal title="Invite a teammate" onClose={onClose}><form className="stack-form" onSubmit={async (event) => { event.preventDefault(); setBusy(true); setError(''); try { await onInvite(email, name, role) } catch (inviteError) { setError(getErrorMessage(inviteError)) } finally { setBusy(false) } }}><p className="modal-intro">They will receive an email to set a password and join every active form in this workspace.</p><label>Full name<input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Jordan Lee" required /></label><label>Email address<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="jordan@company.com" required /></label><label>Role<select value={role} onChange={(event) => setRole(event.target.value as Role)}><option value="user">Team member</option><option value="admin">Administrator</option></select></label>{error && <div className="inline-message error"><ShieldAlert size={16} />{error}</div>}<div className="modal-actions"><button type="button" className="button ghost" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? 'Sending…' : 'Send invitation'}<ArrowRight size={16} /></button></div></form></Modal>
}
