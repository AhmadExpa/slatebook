  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'open' : ''}`}>
        ...
        <nav className="main-nav"><span className="nav-label">Workspace</span>{profile.role === 'admin' && <NavItem icon={<LayoutDashboard size={18} />} label="Overview" active={view === 'overview'} onClick={() => navigate('overview')} />}<NavItem icon={<FileText size={18} />} label={profile.role === 'admin' ? 'Customer records' : 'Lead portal'} active={view === 'records'} onClick={() => navigate('records')} />{profile.role === 'admin' && <NavItem icon={<SlidersHorizontal size={18} />} label="Lead intake" active={view === 'forms'} onClick={() => navigate('forms')} />}{profile.role === 'admin' && <><span className="nav-label admin-label">Admin console</span><NavItem icon={<Users size={18} />} label="Team access" active={view === 'users'} onClick={() => navigate('users')} /><NavItem icon={<BarChart3 size={18} />} label="Exports" active={false} onClick={() => navigate('records')} /></>}</nav>
